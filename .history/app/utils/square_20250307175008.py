...
async def process_square_payment(order_id, amount, payment_method_id):
    ...
    payload = {
        ...
        "delay_duration": "PT0S",  # Set delay to zero seconds
        "autocomplete": True,  # Ensure the payment is completed immediately
    }
...
async def create_square_order(items, location_id):
    ...
    # Default fulfillment data
    default_fulfillment_data = {
        "type": "PICKUP",
        "state": "PROPOSED",
        "pickup_details": {
            "recipient": {
                "display_name": "Customer Name"  # Replace with actual customer name if available
            },
            "schedule_type": "ASAP",  # Or a specific scheduled time
            "prep_time_duration": "PT1H",
        },
    }

    body = {
        "idempotency_key": str(uuid.uuid4()),  # Generating a unique UUID for each order
        "order": {
            "location_id": location_id,
            "line_items": line_items,
            "fulfillments": [
                default_fulfillment_data
            ],  # Adding default fulfillment details
        },
    }
