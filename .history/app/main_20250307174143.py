...
@app.get("/test_payment_processing")
async def test_payment_processing():
    try:
        import logging

        logging.basicConfig(level=logging.INFO)
        logger = logging.getLogger("test_payment_processing")
        ...
        logger.info("Getting Square location ID...")
        location_id = await get_square_location_id()
        if not location_id:
            return {"status": "error", "message": "Failed to get Square location ID"}
        logger.info(f"Location ID: {location_id}")

        # Step 2: Create a test order with a hardcoded item
        # You may need to replace this with a valid item variation ID from your catalog
        test_items = [
            {
                "item_variation_id": "LKXK7KQVTJVNZFBSHACQYX6B",  # Replace with a valid ID if needed
                "quantity": "1",
            }
        ]
        ...
        return {
            ...
            "note": "Check the Square Sandbox Dashboard to view this order and payment.",
        }
