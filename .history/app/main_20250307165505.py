...
async def test_square_payment():
    try:
        ...
        test_items = [
            {
                "item_variation_id": "LKXK7KQVTJVNZFBSHACQYX6B",  # Replace with a known variation ID from your catalog
                "quantity": "1",
            }
        ]
        ...
        # Step 3: Process payment with test nonce
        ...
        return {
            "status": "success" if "payment" in payment_result else "error",
            "order_id": order_id,
            "order_total": order_total,
            "payment_result": payment_result,
            "payment_status": (
                payment_result.get("payment", {}).get("status")
                if "payment" in payment_result
                else "FAILED"
            ),
        }
