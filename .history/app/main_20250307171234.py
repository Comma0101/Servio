...
@app.get("/test-square-order-status")
async def test_square_order_status():
    try:
        import logging

        logging.basicConfig(level=logging.INFO)
        logger = logging.getLogger("test_square_order_status")

        from app.utils.square import retrieve_square_order, get_square_location_id

        # Get the most recent order ID from a test run
        # You can replace this with a specific order ID if you have one
        order_id = input("Enter the order ID to check: ")
        ...
        order_result = await retrieve_square_order(order_id)

        if "error" in order_result:
            logger.error(f"Error retrieving order: {order_result['error']}")
            return {
                "status": "error",
                "message": f"Error retrieving order: {order_result['error']}",
            }
        ...
        return {
            ...
            "order_status": order_result.get("order", {}).get("state"),
            "location_id": location_id,
            "sandbox_dashboard_url": "https://squareupsandbox.com/dashboard/",
            "note": "Check the Square Sandbox Dashboard to view this order. Make sure you're logged into your sandbox account, not your production account.",
        }
...
@app.get("/test-square-create-payment")
async def test_square_create_payment():
    try:
        import logging

        logging.basicConfig(level=logging.INFO)
        logger = logging.getLogger("test_square_create_payment")
        ...
        # Step 2: Create a test order with a specific item
        # You should replace this with an actual item variation ID from your catalog
        test_items = [
            {
                "item_variation_id": "LKXK7KQVTJVNZFBSHACQYX6B",  # Replace with your item ID
                "quantity": "1",
            }
        ]

        logger.info(f"Creating order with items: {test_items}")
        order_result = await create_square_order(test_items, location_id)
        ...
        payment_method_id = "cnon:card-nonce-ok"  # Square sandbox test nonce
        logger.info(f"Processing payment with method ID: {payment_method_id}")

        payment_result = await process_square_payment(
            order_id, order_total, payment_method_id
        )

        if not payment_result or "payment" not in payment_result:
        ...
        logger.info(f"Payment successful with ID: {payment_result['payment']['id']}")
        ...
        return {
            ...
            "payment_details": payment_result,
            "sandbox_dashboard_url": "https://squareupsandbox.com/dashboard/",
            "note": "Check the Square Sandbox Dashboard to view this order and payment. Make sure you're logged into your sandbox account, not your production account.",
        }
