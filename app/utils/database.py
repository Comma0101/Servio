...
import logging
import wave
import audioop # For μ-law to PCM conversion
...
async def upload_audio_to_s3(call_sid, audio_data):
    ...
    try:
        ...
        wav_data_to_upload = audio_data  # Default to original raw data
        conversion_successful = False
        temp_wav_file = f"/tmp/{file_name}" # Define path for cleanup

        try:
            # Attempt to convert μ-law audio_data to 16-bit linear PCM
            pcm_data = audioop.ulaw2lin(audio_data, 2)  # 2 bytes for 16-bit PCM
            ...
            with open(temp_wav_file, 'rb') as f:
                wav_data_to_upload = f.read()
            
            logger.info(f"Successfully converted to 16-bit PCM WAV, size: {len(wav_data_to_upload)} bytes")
            conversion_successful = True
            
        except Exception as conversion_error:
            logger.error(f"Error converting/creating 16-bit PCM WAV file: {str(conversion_error)}")
            logger.info("Falling back to uploading raw mulaw data (will likely be noisy if played as PCM WAV).")
            # wav_data_to_upload remains the original audio_data if conversion failed
        finally:
            if os.path.exists(temp_wav_file):
                os.remove(temp_wav_file)

        # Determine audio format for S3 metadata
        audio_format_in_s3 = 'pcm_16bit' if conversion_successful else 'mulaw_8bit_raw_fallback'
        
        metadata = {
            'call_sid': call_sid,
            'format': audio_format_in_s3,
            'sample_rate': '8000',
            'channels': '1',
        ...
        # S3 client is already initialized globally as s3_client
        # Upload to S3
        logger.info(f"Uploading to S3: bucket={S3_BUCKET}, key={file_name}, format in S3: {audio_format_in_s3}")
        s3_client.put_object(
            Body=wav_data_to_upload,
            Bucket=S3_BUCKET,
            Key=file_name,
            ContentType='audio/wav', # ContentType is still audio/wav
            Metadata=metadata
        )
    ...
    except Exception as e: # This is the outer catch-all for the upload_audio_to_s3 function
        logger.error(f"Overall error in upload_audio_to_s3 for call {call_sid}: {str(e)}")
        import traceback
