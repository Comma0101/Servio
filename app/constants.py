...
CONSTANTS = {
        ...
        "SYSTEM_MESSAGE": (
            ...
            "4.  Guided Configuration: Follow the guided item configuration process. Ask the exact question from `message_for_agent` and use the tool specified in `tool_to_use` until the item is complete (`status: ITEM_COMPLETE`).\n"
            "5.  Final Order: When the customer says they are done, call `place_order` to submit the order to the POS system.\n\n"
            "# CRITICAL RULES\n"
